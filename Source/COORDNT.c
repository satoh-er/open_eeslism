	...
	POLYGN *poly, TREE *tree, OBS *obs, BBDP *BDP, P_MENN *lp)
{
	...
	for (i = 0; i < bdpn; i++){
		if (test == 1)
			printf("BDP=%s\n", BDP[i].bdpname);
		sWb = sin(BDP[i].Wb*rad);
		cWb = cos(BDP[i].Wb*rad);
		sWa = sin(-BDP[i].Wa*rad);
		cWa = cos(-BDP[i].Wa*rad);
		...
		if (BDP[i].sumsblk != 0){
			for (j = 0; j < BDP[i].sumsblk; j++){
				...
				if (strcmp(BDP[i].SBLK[j].sbfname, "HISASI") == 0){
					...
					chWA = cos((BDP[i].SBLK[j].WA - BDP[i].Wb)*rad);
					shWA = sin((BDP[i].SBLK[j].WA - BDP[i].Wb)*rad);
					...
					lp[k].e.Z = cos(lp[k].wb*rad);
					lp[k].e.Y = -sin(lp[k].wb*rad)*cos(lp[k].wa*rad);
					lp[k].e.X = -sin(lp[k].wb*rad)*sin(lp[k].wa*rad);
				...
				/*---------------------------------------*/
					...
					chWA = cos((90.0 - BDP[i].Wb)*rad);
					shWA = sin((90.0 - BDP[i].Wb)*rad);
					...
					/*--法線ベクトルの算出　HOUSEN2関数を使うと、向きが逆になるので、変更 091128 higuchi --*/
					lp[k + 1].e.Z = cos(lp[k + 1].wb*rad);
					lp[k + 1].e.Y = -sin(lp[k + 1].wb*rad)*cos(lp[k + 1].wa*rad);
					lp[k + 1].e.X = -sin(lp[k + 1].wb*rad)*sin(lp[k + 1].wa*rad);
					...
					/*--法線ベクトルの算出　HOUSEN2関数を使うと、向きが逆になるので、変更 091128 higuchi --*/
					lp[k + 2].e.Z = cos(lp[k + 2].wb*rad);
					lp[k + 2].e.Y = -sin(lp[k + 2].wb*rad)*cos(lp[k + 2].wa*rad);
					lp[k + 2].e.X = -sin(lp[k + 2].wb*rad)*sin(lp[k + 2].wa*rad);
					...
					/*--法線ベクトルの算出　HOUSEN2関数を使うと、向きが逆になるので、変更 091128 higuchi --*/
					lp[k + 3].e.Z = cos(lp[k + 3].wb*rad);
					lp[k + 3].e.Y = -sin(lp[k + 3].wb*rad)*cos(lp[k + 3].wa*rad);
					lp[k + 3].e.X = -sin(lp[k + 3].wb*rad)*sin(lp[k + 3].wa*rad);
					...
					/*--法線ベクトルの算出　HOUSEN2関数を使うと、向きが逆になるので、変更 091128 higuchi --*/
					lp[k + 4].e.Z = cos(lp[k + 4].wb*rad);
					lp[k + 4].e.Y = -sin(lp[k + 4].wb*rad)*cos(lp[k + 4].wa*rad);
					lp[k + 4].e.X = -sin(lp[k + 4].wb*rad)*sin(lp[k + 4].wa*rad);
				...
				/*---------------------------------------------------*/
					...
					b = BDP[i].y0 + BDP[i].SBLK[j].x*sWa + BDP[i].SBLK[j].y*cWb*cWa;
					c = BDP[i].z0 + BDP[i].SBLK[j].y*sWb;
					csWA = cos((-BDP[i].Wa - BDP[i].SBLK[j].WA)*rad);
					ssWA = sin((-BDP[i].Wa - BDP[i].SBLK[j].WA)*rad);
					csWb = cos((90.0 - BDP[i].Wb)*rad);
					ssWb = sin((90.0 - BDP[i].Wb)*rad);
	...
	for (i = 0; i < obsn; i++){
		if (strcmp(obs[i].fname, "rect") == 0){
			...
			caWb = cos(obs[i].Wb*rad);
			saWa = sin(-obs[i].Wa*rad);
			saWb = sin(obs[i].Wb*rad);
			caWa = cos(-obs[i].Wa*rad);
			...
			cbWa = cos(-obs[i].Wa*rad);
			sbWa = sin(-obs[i].Wa*rad);
	...
	for (i = 0; i < treen; i++){
		if (strcmp(tree[i].treetype, "treeA") == 0){
			...
			lp[k].P[2].X = tree[i].x + (tree[i].W3 * 0.5) * cos(45 * rad);
			lp[k].P[2].Y = tree[i].y - (tree[i].W3 * 0.5) * sin(45 * rad);
			lp[k].P[2].Z = lp[k].P[1].Z;
			lp[k].P[3].X = tree[i].x + (tree[i].W2 * 0.5) * cos(45 * rad);
			lp[k].P[3].Y = tree[i].y - (tree[i].W2 * 0.5) * sin(45 * rad);
			lp[k].P[3].Z = tree[i].z + tree[i].H1;

			HOUSEN2(&lp[k].P[0], &lp[k].P[1], &lp[k].P[2], &lp[k].e);
			lp[k].wb = acos(lp[k].e.Z) * (180 / pai);
			/*--法線ベクトルの算出　HOUSEN2関数を使うと、向きが逆になるので、変更 091128 higuchi --*/
			lp[k].e.Z = cos(lp[k].wb*rad);
			lp[k].e.Y = -sin(lp[k].wb*rad)*cos(lp[k].wa*rad);
			lp[k].e.X = -sin(lp[k].wb*rad)*sin(lp[k].wa*rad);
			...
			lp[k].P[2].X = tree[i].x + (tree[i].W4 * 0.5) * cos(45 * rad);
			lp[k].P[2].Y = tree[i].y - (tree[i].W4 * 0.5) * sin(45 * rad);
			lp[k].P[2].Z = lp[k].P[1].Z;
			lp[k].P[3].X = tree[i].x + (tree[i].W3 * 0.5) * cos(45 * rad);
			lp[k].P[3].Y = tree[i].y - (tree[i].W3 * 0.5) * sin(45 * rad);
			lp[k].P[3].Z = lp[k].P[0].Z;
	...
	for (i = 0; i < polyn; i++){
		...
		if (strcmp(poly[i].polyknd, "OBS") == 0){
			...
			HOUSEN2(&lp[k].P[0], &lp[k].P[1], &lp[k].P[2], &lp[k].e);
			lp[k].wb = acos(lp[k].e.Z) * (180 / pai);
			lp[k].wa = asin(lp[k].e.X / sin(lp[k].wb*rad)) * (180 / pai);
			/*--法線ベクトルの算出　HOUSEN2関数を使うと、向きが逆になるので、変更 091128 higuchi --*/
			lp[k].e.Z = cos(lp[k].wb*rad);
			lp[k].e.Y = -sin(lp[k].wb*rad)*cos(lp[k].wa*rad);
			lp[k].e.X = -sin(lp[k].wb*rad)*sin(lp[k].wa*rad);
...
void OP_COORDNT(int *opn, int bdpn, BBDP *BDP, P_MENN *op, int polyn, POLYGN *poly)
{
	...
	for (l = 0; l < bdpn; l++){
		...
		cWa = cos(-BDP[l].Wa*rad);
		sWa = sin(-BDP[l].Wa*rad);
		cWb = cos(BDP[l].Wb*rad);
		sWb = sin(BDP[l].Wb*rad);
		...
		for (j = v; j < sumop; j++){
			...
			op[j].e.Z = cos(op[j].wb*rad);
			op[j].e.Y = -sin(op[j].wb*rad)*cos(op[j].wa*rad);
			op[j].e.X = -sin(op[j].wb*rad)*sin(op[j].wa*rad);
	...
	for (i = 0; i < polyn; i++){
		if (strcmp(poly[i].polyknd, "RMP") == 0){
			...
			op[sumop].wb = acos(op[sumop].e.Z) * (180 / pai);
			op[sumop].wa = asin(op[sumop].e.X / sin(op[sumop].wb*rad)) * (180 / pai);

			/*--ポリゴンの法線ベクトルが逆で出てしまうのを修正 091128 higuchi--*/
			op[sumop].e.Z = cos(op[sumop].wb*rad);
			op[sumop].e.Y = -sin(op[sumop].wb*rad)*cos(op[sumop].wa*rad);
			op[sumop].e.X = -sin(op[sumop].wb*rad)*sin(op[sumop].wa*rad);
